...
- (void)handleClick:(UIButton *)button {
    if (_currentButton == button && _currentButton.selected) {
        _currentButton.selected = NO;
        if (self.keyboardStateChangedBlock) self.keyboardStateChangedBlock(DDYKeyboardStateNone);
    } else {
        _currentButton.selected = NO;
        _currentButton = button;
        _currentButton.selected = YES;
        if (self.keyboardStateChangedBlock) self.keyboardStateChangedBlock((DDYKeyboardState)button.tag);
    }
}
